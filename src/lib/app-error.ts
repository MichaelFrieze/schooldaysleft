...
// 	if (!isServerRuntime()) {
// 		throw new Error(
// 			"Attempted to construct AppError on client runtime. AppError may only be created on the server.",
// 		);
// 	}
// }
...
export function createApError(opts: {
  appErrorCode: AppErrorCode
  message?: string
  cause?: unknown
}): AppError {
...
export function assertOrThrowAppError(
  condition: unknown,
  appErrorCode: AppErrorCode,
  message?: string,
): asserts condition {
