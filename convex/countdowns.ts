...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.weeklyDaysOff.length > 0) {
      const invalidDays = args.weeklyDaysOff.filter((day) => day < 0 || day > 6)
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.name && args.name !== existingCountdown.name) {
      const nameToCheck = args.name // narrow without non-null assertion
      const duplicateCountdown = await ctx.db
        .query('countdowns')
        .withIndex('by_user_name', (q) =>
          q.eq('userId', userId).eq('name', nameToCheck),
        )
        .unique()
